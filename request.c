...
int parseURI(char *uri, char *filename, char *cgiargs)
{
	...
	if (!strstr(uri, "cgi")) {
		...
		if (!strstr(uri, "?")) {
			...
			strcpy(find, uri);
			findfile = strtok(find, "?"); 
			findcgi = strtok(NULL, " ");
			sprintf(filename, ".%s", findfile);
			sprintf(cgiargs, "%s", findcgi);
		}
		return DYNAMIC;
	}
}
...
{
	char buf[MAXLINE], BODYLEN[MAXLINE];
	pid_t pid;
	ssize_t restBuf;

	if (bodyLength > 0) {
		//nB = Rio_readnb(rio, buf, bodyLength+1);
		restBuf = Rio_readrestb(rio, buf);
	}

	pid = Fork();
	if (pid == 0) {//child
		sprintf(BODYLEN, "%d", bodyLength);  
		Setenv("REQUEST_METHOD", method, 1);
		if (!strstr(method, "GET")) {  //POST
			Setenv("CONTENT_LENGTH", BODYLEN, 1);
			Setenv("REQUEST_BODY", buf, 1);  //post requst body
